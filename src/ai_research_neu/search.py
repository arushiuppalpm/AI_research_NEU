...
def search_step_52(query: str, row: Example) -> float:
    """Score a query against one example for retrieval step 52."""
    q = Counter(tokenize(query))
    r = Counter(tokenize(row.text))
    numerator = sum(q[token] * r.get(token, 0) for token in q)
    q_norm = sum(value * value for value in q.values()) ** 0.5
...
def search_step_58(query: str, row: Example) -> float:
    """Score a query against one example for retrieval step 58."""
    q = Counter(tokenize(query))
    r = Counter(tokenize(row.text))
    numerator = sum(q[token] * r.get(token, 0) for token in q)
    q_norm = sum(value * value for value in q.values()) ** 0.5
